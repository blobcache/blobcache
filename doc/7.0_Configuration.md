...
> Typically $HOME/.local/share/blobcache is used as the state dir.
...
PEER_LOC            Group file associating PeerIDs with groups of hostnames.
```
...
## Peer Locations
The `PEER_LOC` file is a group file that maps NodeIDs to network addresses.
It tells a Node where to find other Nodes on the network, without requiring a full Endpoint to be specified in every volume spec.

When a Peer volume is created (using a NodeID and volume OID, but no IP address), the `PEER_LOC` file is consulted to resolve the NodeID to a connectable address.
Each entry maps a NodeID to one or more `host:port` pairs.
The host can be a domain name (e.g. `localhost:6025`, `myserver.example.com:6025`) or an IP address — DNS resolution is performed when connecting.

If multiple addresses are listed for a peer, they are tried in order until one succeeds.

*Example 7*
```
NNl0kKyillGqHtgVLevR-MkwJkWz7ry3ZpxEWEmzTZ- myserver.example.com
fnrwKB7Xd94r4156CCTmLskoDsrOewGPBGIwj8R1FuM localhost:6025
```

The default BCP port is `6025`.
If no port is explicitly provided, that will be used instead.
The `PEER_LOC` file is read at daemon startup.
...
### Local API
The Local API is usually served over a UNIX socket, although it can be served over any kind of stream socket, like TCP as well.

#### UNIX
The UNIX socket API is a more efficient binary protocol than HTTP.

Blobcache can listen on a unix socket using the `--serve-ipc` flag.

*Example 8*
```
$ blobcache daemon-ephemeral --serve-ipc ./blobcache.sock
```

#### HTTP
It is dangerous to serve the HTTP API to the network, as it allows any client to act as the Blobcache Node.

This parameter is controlled with the `--serve-http` flag.
