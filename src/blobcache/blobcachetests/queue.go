...
func QueueAPI(t *testing.T, setup func(testing.TB) (blobcache.Service, blobcache.Handle)) {
	// Need tests for:
	// - max bytes
	// - max handles
	...
	t.Run("MaxBytes", func(t *testing.T) {
		t.Parallel()
		t.Skip("queue max-bytes limit not defined")
	})
	t.Run("MaxHandles", func(t *testing.T) {
		t.Parallel()
		t.Skip("queue max-handles limit not defined")
	})
