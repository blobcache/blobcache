...
Blobcache Volumes, being content-addressable stores themselves, work well with Git, which is built on a content addressable store.
This guide walks through creating a Volume and then using Git to push and pull data into and out of the Volume.
...
There is a blobcache command to print out a URL in the format that Git expects.
```shell
$ blobcache git url-for mygitremote
```

To add a new remote to git:
```shell
$ git remote add myBlobcacheVolume bc::JcJvfY9tFsfkHSwoMT8IEoSq1ZfxVYBAwpBRvJ0uUJA:194EA0199098AD3801D7EEA721CBEC6F
```
...
## Done
That's it, now you can push and pull Git data in and out of Blobcache.
You might also want to try Vault (encrypted) Volumes and remote Volumes for creating securely storing your Git data on another machine.
