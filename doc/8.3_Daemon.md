...
## `blobcache daemon run --state <dir> [--serve-http proto://addr] [--serve-ipc path] [--net host:port]`
...
## `blobcache daemon ephemeral [--serve-http proto://addr] [--serve-ipc path] [--net host:port]`
...
## `blobcache daemon mig-link-hashes --state <dir>`
Migrates legacy sha3-based link token hashes to per-volume hashes for supported namespace schemas.
