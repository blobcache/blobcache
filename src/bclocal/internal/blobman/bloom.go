...
// bloomFilter2048 is a bloom filter with 2048 bits.
//
...
type bloomFilter2048 struct {
	data [32]uint64
}

func (bf *bloomFilter2048) set(update [256]byte) {
...
func (bf *bloomFilter2048) get(fp [256]byte) bool {
...
func (bf *bloomFilter2048) bitPattern(k Key) [256]byte {
	var pat [256]byte
	// need 8 hashes
	var hashes [8]uint64
	bloomHash(k, &hashes)
...
func (bf *bloomFilter2048) add(k Key) {
	bf.set(bf.bitPattern(k))
}

func (bf *bloomFilter2048) contains(k Key) bool {
	return bf.get(bf.bitPattern(k))
}
